...
type Props = {
  ...
  paginatedDaftar: TagihanLain[];
  sortField: SortField;
  sortAsc: boolean;
  toggleSort: (f: SortField) => void;
  onSiswaClick: (s: TagihanLain["siswa"]) => void;
              ...
              paginatedDaftar.map((t) => {
                ...
                return (
                  <tr key={t.id} className={`hover:bg-violet-50/60 [&>td]:hover:bg-violet-50/60 ${selectedIds.has(t.id) ? "bg-red-50/40 [&>td]:bg-red-50/40" : ""}`}>
                    ...
                    <td className="border-b border-slate-100 bg-white px-5 py-4 align-middle text-sm">
                      <span className="font-semibold text-ink-900">{t.jenisTagihanLain?.nama || "-"}</span>
                          ...
                          <>
                            ...
                            >
                              {verifyingId === t.id ? (
                                ...
                                <>
                                  <IconCheck width={14} height={14} /> Lunas
                                </>
                              )}
                            </button>
                            <button
